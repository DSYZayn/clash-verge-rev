...
const SettingVergeAdvanced = ({ onError: _ }: Props) => {
  ...
  const onCheckUpdate = async () => {
    try {
      ...
      updateLastCheckTime()
      // Keep the shared update query in sync so the dialog below (and the
      // layout update badge) render this fresh result instead of stale cache.
      setCacheData(['checkUpdate'], info)
      if (!info?.available) {
        showNotice.success(
